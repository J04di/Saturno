...
export default {
    ...
    handled: async (venus, { m }) => {
        ...
        await venus.sendMessage(m.chat, { video: result, mimetype: "video/mp4", gifPlayback: true, caption: `@${m.sender.split("@")[0]} le dió una palisa a @${mentionedJid.split("@")[0]}`, mentions: [m.sender, mentionedJid] }, { quoted: m });
    }
}